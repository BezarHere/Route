...
namespace route
{
	// TODO: add equal approximation functions for floating-point errors
	...
	struct Basis
	{
		...
		inline constexpr Basis( const Vec3f xx, const Vec3f yy, const Vec3f zz )
			: xdir{ xx }, ydir{ yy }, zdir{ zz } {
		}

		inline Basis( const Vec3f euler ) {
			set_angle( euler );
		}
		...
		/// !MAJOR FAULTY!
		inline Vec3f get_rotation() const {
			Vec3f euler;
		...
		inline void set_angle( const Vec3f euler ) {
			float c, s;
		...
		inline void set_rotation( Vec3f axis, float angle ) {
			const Vec3f axis_sq = axis.squared();
		...
		inline void rotate( const Vec3f p_euler ) {
		...
		union
		{
			...
			Vec3f v[ 3 ]{ { 1.f, 0.f, 0.f }, { 0.f, 1.f, 0.f }, { 0.f, 0.f, 1.f } };
		};
	};
