...
namespace route
{
  ...
  public:
    ...
    APIState() {
      ...
      const auto frag_shader =
        ...
        ;
      shader = ShaderProgram( Shader( vertex_shader, ShaderType::Vertex ), Shader( frag_shader, ShaderType::Fragment ) );
  ...
  void Renderer::render( const Application &app ) {
    ...
    set_shader_program( m_api_state->shader );
  ...
  void Renderer::set_shader_program( const ShaderProgram &shader ) {
    m_state.ShaderProg = shader.get_id();
  }
...
#ifdef GAPI_GL
    GAPI_IF_GL( Application::graphics_api() == GraphicsAPI::OpenGL ) {
      glUseProgram( _rt::to_gl_name( m_state.ShaderProg ) );
      // TODO: check for errors?
    }
#endif // GAPI_GL
  }
